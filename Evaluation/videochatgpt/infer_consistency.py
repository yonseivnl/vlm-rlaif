...
sys.path.append(os.path.dirname(os.path.dirname(__file__))) # Want /dataset/dcahn/llms/llava/LLaVA_Video_temp
print(sys.path)
...
def run_inference(args):
    ...
    for idx, sample in tqdm(enumerate(gt_contents), total=len(gt_contents)):
        ...
        sample_set["question_1"] = question_1
        sample_set["question_2"] = question_2
        sample_set["answer"] = answer
        
        # Get conversation
        for qid, question in enumerate([question_1, question_2]):
            ...
            sample_set[f'pred{qid+1}'] = outputs
