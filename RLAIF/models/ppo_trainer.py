...
class PPOTrainer(RLTrainer):
    ...
    def rollout(self, queries_data) -> Dict[str, torch.Tensor]:
        ...
        for batch_idx, batch in tqdm.tqdm(
            ...
            respond_outputs = unwrapped_policy.respond(queries, query_attn_masks, images, temperature=self.args.temperature)
            ...
            sequences = [torch.concat((query, response), dim=0) for query, response in zip(queries, responses)]
            sequences = torch.stack(sequences, dim=0)

            sequences = remove_pad_and_left_pad(sequences, pad_token_id=self.tokenizer.pad_token_id,)

            if self.reward_model_prompt is not None:
                reward_model_prompt = (self.reward_model_prompt.reshape(1, -1).repeat(len(sequences), 1).to(self.accelerator.device))
                sequences = torch.cat((sequences, reward_model_prompt), dim=1)

            clean_sequences = sequences.detach().clone()
            clean_sequences[clean_sequences == IMAGE_TOKEN_INDEX] = self.tokenizer.eos_token_id
            text_sequences = self.tokenizer.batch_decode(clean_sequences, skip_special_tokens=False, clean_up_tokenization_spaces=False,)
            if self.accelerator.is_main_process:
                print("=" * 20)
                print(text_sequences[0].split("<unk><s> ")[-1])
                print("=" * 20)
                print("Filename: ", self.uid_to_fname[str(uid[0].detach().item())])
                print("=" * 20)

            non_pad_mask = responses.ne(self.tokenizer.pad_token_id)
            non_pad_seq_len = (non_pad_mask.sum(dim=1).float().to(self.accelerator.device))
            ...
            length_bonus = length_bonus * length_bonus_multiplier.to(length_bonus.device).reshape(length_bonus.shape).to(length_bonus.dtype)
            ...
            if sub_batch_size is None or sub_batch_size == batch_size_per_device:
                ...
                for sub_batch_idx in range(batch_size_per_device // sub_batch_size):
                    sub_batch = {key: value[sub_batch_idx * sub_batch_size : (sub_batch_idx + 1) * sub_batch_size] for key, value in rollouts_batch.items()}
                    
                    sub_batch_policy_outputs = self.policy(**sub_batch, temperature=self.args.temperature)
                    policy_outputs_list.append(sub_batch_policy_outputs)

                policy_outputs = common_utils.merge_dict(policy_outputs_list, merge_fn=torch.cat)
            ...
            if sub_batch_size is None or sub_batch_size == batch_size_per_device:
                ...
                for sub_batch_idx in range(batch_size_per_device // sub_batch_size):
                    ...
                    sub_batch_ref_policy_outputs = self.ref_policy(**sub_batch, temperature=self.args.temperature)
                    ref_policy_outputs_list.append(sub_batch_ref_policy_outputs)


                ref_policy_outputs = common_utils.merge_dict(ref_policy_outputs_list, merge_fn=torch.cat)
                
                del sub_batch_ref_policy_outputs
                del ref_policy_outputs_list
                del sub_batch

            policy_outputs = common_utils.unpack_dict(policy_outputs, keys=("logprobs", "values", "entropies"), return_type=dict,)

            ref_policy_outputs = common_utils.unpack_dict(ref_policy_outputs, keys=("logprobs", "entropies"), return_type=dict)
            ...
            if sub_batch_size is None or sub_batch_size == batch_size_per_device:
                reward_outputs = self.reward_model(input_ids=sequences, attention_mask=sequences_attention_mask, images=reward_images,)
            else:
                assert batch_size_per_device % sub_batch_size == 0
                ...
                for sub_batch_idx in range(batch_size_per_device // sub_batch_size):
                    ...
                    sub_batch_reward_outputs = self.reward_model(input_ids=sequences[idx_start:idx_end], attention_mask=sequences_attention_mask[idx_start:idx_end], images=reward_images[idx_start:idx_end],)
                    reward_outputs_list.append(sub_batch_reward_outputs)

                reward_outputs = common_utils.merge_dict(reward_outputs_list, merge_fn=torch.cat)
                
                del reward_outputs_list
                del sub_batch_reward_outputs

            reward_outputs = self.post_reward(reward_outputs, responses, penalize_no_stop_token=self.args.penalize_no_stop_token, 
                                              relative_stop_token_penalty=self.args.relative_stop_token_penalty, has_stop_token=has_stop_token,)
        ...
        advantages = self._estimate_advantage(rewards=rollouts["shaped_rewards"].to(self.accelerator.device), values=rollouts["values"].to(self.accelerator.device),)
        advantages = {key: value.cpu() for key, value in advantages.items()}
        
        return {**rollouts, **advantages}

    def post_reward(self, reward_outputs: Dict[str, torch.Tensor], responses: torch.Tensor,
                    penalize_no_stop_token: bool, relative_stop_token_penalty: bool, has_stop_token: List[bool],) -> Dict[str, torch.Tensor]:
        ...
        if penalize_no_stop_token:
            ...
            if relative_stop_token_penalty:
                rewards = (rewards + (~has_stop_token).float() * self.args.penalty_reward_value)
            else:
        ...
        def get_validity_mask(sequences: torch.Tensor, end_token_id: int) -> torch.Tensor:
    ...
    def compute_policy_loss(self, rollouts: Dict[str, torch.Tensor]) -> Tuple[torch.Tensor, Dict]:
        (
            values,
            old_logprob,
            returns,
        ...
        outputs = self.policy(queries, query_attn_masks, responses, images, temperature=self.args.temperature, mode="policy",)
        ...
        stats = dict(loss=dict(policy=pg_loss), policy=dict(entropy=entropy, approxkl=approxkl, clipfrac=pg_clipfrac), returns=dict(mean=return_mean, var=return_var),)
        return loss, common_utils.flatten_dict(stats, sep="/", postprocess_fn=lambda x: x.detach())

    def compute_value_loss(self, rollouts: Dict[str, torch.Tensor]) -> Tuple[torch.Tensor, Dict]:
        (
            values,
            old_logprob,
            returns,
        ...
        outputs = self.policy(queries, query_attn_masks, responses, images, reward_images, temperature=self.args.temperature, mode="value",)

        vpred = outputs["values"]
        vpredclipped = torch.clamp(vpred, min=values - self.args.cliprange_value, max=values + self.args.cliprange_value,)
        ...
        stats = dict(loss=dict(value=vf_loss), val=dict(vpred=vpred.mean(), error=((vpred - returns) ** 2).mean(), clipfrac=vf_clipfrac, mean=value_mean, var=value_var,),)
    ...
    def record_step_stats(self, train_stats, rollouts, step_idx, **kwargs):
        ...
        if self.accelerator.is_main_process:
            ...
            if self.args.output_dir is not None:
                ...
                with open(os.path.join(rollout_log_dir, f"step_{step_idx}.json"), "w",) as f:
    ...
    def save_model(self, output_dir: Optional[str] = None, give_rw_access=True, check_corrupted=True,):
        ...
        if global_rank == 0:
            ...
            save_adapters(policy_model, peft_model_path, adapter_names=["lora_policy"],)

            pytorch_model_paths = glob.glob(os.path.join(output_dir, "pytorch_model*.bin"))
            ...
            save_adapters(value_model.base_model, peft_model_path, adapter_names=["lora_value"],)
            torch.save(value_model.value_head.state_dict(), os.path.join(output_dir, VALUE_HEAD_NAME),)
    ...
    def resume_training(self, checkpoint_dir):
        ...
        if os.path.exists(optimizer_path):
            load_paged_optimizer_state_dict(self.optimizer.optimizer, torch.load(optimizer_path, map_location="cpu",),)
...
def smart_tokenizer_and_embedding_resize(num_new_tokens: int, tokenizer: transformers.PreTrainedTokenizer, model: transformers.PreTrainedModel,):
    if num_new_tokens > 0:
...
def make_models(tokenizer: transformers.PreTrainedTokenizer, args, accelerator: accelerate.Accelerator, num_new_tokens: int = 0, resume_from_checkpoint: Optional[str] = None,) -> dict:
    def make_generative_policy(adapter_name, is_trainable, reuse_base_model=True, resume_path=None):
    ...
    def make_reward_model(adapter_name, is_trainable, reuse_base_model=True, resume_path=None):
        ...
        smart_tokenizer_and_embedding_resize(num_new_tokens, tokenizer, model.backbone_model)
        return model

    policy_resume_path = None
    if resume_from_checkpoint:
        policy_resume_path = os.path.join(resume_from_checkpoint, ADAPTER_MODEL_DIR, "lora_policy")
    policy = rl_models.make_policy_with_base_model(args, make_generative_policy(adapter_name="lora_policy", is_trainable=True, resume_path=policy_resume_path,), tokenizer, adapter_name="lora_policy",)

    value_resume_path = None
    value_head_resume_path = None
    if resume_from_checkpoint:
        value_resume_path = os.path.join(resume_from_checkpoint, ADAPTER_MODEL_DIR, "lora_value")
    ...
    if args.init_value_with_reward:
        ...
        value_model = rl_models.make_value_with_base_model(
            args,
            make_reward_model(adapter_name="lora_value", is_trainable=True, resume_path=value_resume_path,).backbone_model,
            tokenizer, adapter_name="lora_value",)
    else:
        ...
        value_model = rl_models.make_value_with_base_model(
            args,
            make_generative_policy(adapter_name="lora_value", is_trainable=True, resume_path=value_resume_path,), 
            tokenizer, adapter_name="lora_value",)

    if value_head_resume_path:
        value_model.value_head.load_state_dict(torch.load(value_head_resume_path, map_location="cpu"))
    ...
    ref_policy = rl_models.make_policy_with_base_model(args, make_generative_policy(adapter_name="lora_ref_policy", is_trainable=False,), tokenizer, adapter_name="lora_ref_policy",)

    reward_model = make_reward_model(adapter_name="lora_reward", is_trainable=False,)

    if args.vision_tower is not None:
        reward_model.backbone_model.config.image_aspect_ratio = args.image_aspect_ratio
        reward_model.backbone_model.config.image_grid_pinpoints = (args.image_grid_pinpoints)
...
def whiten(values: torch.Tensor, shift_mean=True, epsilon=1e-8, async_stats="full_batch") -> torch.Tensor:
    ...
    if async_stats == "full_batch":
        ...
        if need_sync:
            tensor_list = [torch.zeros_like(values_for_statistics) for _ in range(world_size)]
    ...
    if async_stats in ["full_batch", "per_gpu"]:
        assert (values_for_statistics.size(0) >= 8), f"Internal error: Minibatch size {values.size(0)} is insufficient for whitening."
