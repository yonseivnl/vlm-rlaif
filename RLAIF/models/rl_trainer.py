...
class RLTrainer(object):
    def __init__(
        ...
        with open('/dataset/llms/LLaVA_RLHF/LLaVA_Video-RLHF/dataset/videochatgpt99k_uid_to_fname.json') as f: # FIXME need to be flexible!
            self.uid_to_fname = json.load(f)
    ...
    def create_optimizer_and_scheduler(self, num_training_steps: int):
        optimizer = create_optimizer(args=self.args, model=self.policy, optimizer=self.optimizer)
        lr_scheduler = create_scheduler(args=self.args, optimizer=optimizer, lr_scheduler=self.lr_scheduler, num_training_steps=num_training_steps,)
    ...
    def train(self, resume_training_ckpt: Optional[str] = None):
        ...
        total_epochs = self.args.total_epochs # Total epochs -> 4 
        total_episodes = len(self.train_dataset) * total_epochs  # noqa // -> total iterations
        total_steps = total_episodes // self.args.rollout_batch_size  # noqa // roll_out_batch_size 512
        logger.warning(f"***Training starts***\n" f"Total epochs: {total_epochs} => Total episodes: {total_episodes} => Total steps: {total_steps}")
        ...
        if resume_training_ckpt is not None:
            ...
            print(f"Resuming training from {resume_training_ckpt} at step {skipping_steps}.")

        infinite_train_dataloader = self.get_train_dataloader()
        for step_idx in tqdm.tqdm(range(FIRST_STEP_IDX, total_steps + FIRST_STEP_IDX), disable = not self.accelerator.is_main_process, desc="steps", total=total_steps,):
    ...
    def _log_batch_size(self, loader: DataLoader, loader_name):
        ...
        logger.warning(f"Batch size of {loader_name} dataloader: {batch_size}",)

    def get_train_dataloader(self):
        logger.warning(f"Train dataset size: {len(self.train_dataset)}",)  # noqa
        train_dataloader = DataLoader(dataset=self.train_dataset, collate_fn=self.data_collator, batch_size=self.args.rollout_per_device_batch_size, shuffle=True, drop_last=True,)
    ...
    def get_rollouts_dataloader(self, rollouts: Dict[str, torch.Tensor], shuffle=True, drop_last=True, keys=None):
        if keys is None:
        ...
        rollouts_dataloader = DataLoader(dataset=rollouts_dataset, batch_size=self.args.step_per_device_batch_size, collate_fn=collate_rollouts, shuffle=shuffle, drop_last=drop_last,)
    ...
    def get_policy_meta_prompts(self):
        ...
        policy_meta_prompts = utils.make_meta_prompts(policy_meta_prompt_pattern,)
    ...
    def get_reward_meta_prompts(self):
        ...
        reward_meta_prompts = utils.make_meta_prompts(reward_meta_prompt_pattern,)
        ...
        if "{Dimensions}" in reward_meta_prompt:
            ...
            if self.args.data_config.max_principles is not None:
                if "weight" not in principle_collection[0]:
                    ...
                    remaining_weights = [item["weight"] for item in principle_collection]
                    remaining_idx = list(range(len(dimension_str)))

                    sampled_dimension_str = []
                    while (len(sampled_dimension_str) < self.args.data_config.max_principles):
                        sampled_idx = random.choices(list(range(len(remaining_idx))), weights=remaining_weights)[0]
                        sampled_dimension_str.append(dimension_str[remaining_idx[sampled_idx]])
            ...
            dimension_str = "\n".join(dimension_str)
            reward_meta_prompt = reward_meta_prompt.replace("{Dimensions}", dimension_str)

        return reward_meta_prompt.format(Input=inputs, Output=outputs,)
...
def truncate_after_eos_with_padding(completions, eos_token_id, pad_token_id, additional_tokens=None):
    # We truncate tokens after eos_token_id
    clean_completions = completions.tolist()
    for idx, completion in enumerate(clean_completions):
        try:
        ...
        if end_idx is not None:
            ...
            if end_idx + 1 < len(completion):
                clean_completions[idx] = clean_completions[idx] + [pad_token_id] * (len(completion) - end_idx - 1)

    clean_completions = torch.tensor(clean_completions, dtype=torch.long, device=completions.device)
    return clean_completions
