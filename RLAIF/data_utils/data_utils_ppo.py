...
class QueryResponseDataset(Dataset):
    ...
    def __init__(
        ...
        queries = [preprocess([__s], tokenizer, has_image=True, mask_target=False, query_len=query_len,)["input_ids"] for __s in tqdm.tqdm(_s)]
        queries = [torch.tensor(query, dtype=torch.long).view(-1)[:-3] for query in queries]
    ...
    def __getitem__(self, idx):
        ...
        image_files = self.list_dict_data[idx]["images"]
        uid = self.list_dict_data[idx]["uid"]
        image_folder = self.data_args.image_folder
        processor = self.data_args.image_processor
        reward_processor = self.data_args.reward_image_processor

        if False: # "caption_type" in self.list_dict_data[idx]:
            caption_type = self.list_dict_data[idx]["caption_type"]
        else:
            caption_type = 1

        if False: # "length_bonus" in self.list_dict_data[idx]:
        ...
        if self.data_args.image_aspect_ratio == "pad":
            ...
            images = [processor.preprocess(image, return_tensors='pt')['pixel_values'][0].to(torch.bfloat16) for image in images]

            reward_images = [expand2square(original_image, tuple(int(x*255) for x in processor.image_mean)) for original_image in original_images]
            reward_images = [reward_processor.preprocess(reward_image, return_tensors='pt')['pixel_values'][0].to(torch.bfloat16) for reward_image in reward_images]
            
        else:
            images = [processor.preprocess(image, return_tensors='pt')['pixel_values'][0].to(torch.bfloat16) for image in images]
            reward_images = [reward_processor.preprocess(reward_image, return_tensors='pt')['pixel_values'][0].to(torch.bfloat16) for reward_image in reward_images]

        return_dict["images"] = torch.stack(images)
        return_dict["reward_images"] = torch.stack(reward_images)
        return_dict["uid"] = torch.tensor(uid, dtype=torch.long)
        return_dict["caption_types"] = torch.tensor(caption_type, dtype=torch.long)
        return_dict["length_bonus_multiplier"] = torch.tensor(length_bonus, dtype=torch.float)
...
def make_rl_data_module(
    ...
    if data_args.dataset_path.endswith("json"): # True
        train_instructions = datasets.load_dataset("json", data_files=data_args.dataset_path)
    else:
        train_instructions = datasets.load_dataset(data_args.dataset_path, data_args.dataset_name)
    
    train_df = pd.concat([pd.DataFrame(train_instructions[split]) for split in data_args.train_splits])
